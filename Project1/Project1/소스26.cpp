...
public:
	...
	bool CAS(int old_value, unsigned int old_st, int value, unsigned int new_st) {
		int old_v = (old_value & 0x3FFFFFFF) | old_st;
		int new_v = (value & 0x3FFFFFFF) | new_st;
		return atomic_compare_exchange_strong(reinterpret_cast<volatile atomic_int*>(&slot), &old_v, new_v);
	}
	void Busy();
		...
		for(int j = 0; j < MAX_LOOP; ++j) {	// 몇번안에 성공 못하면 타임 아웃 시켜줘야 함
			...
			case ST_EMPTY:
				if (true == CAS(ST_EMPTY, value, ST_WAITING)) { //slot = value;
					...
					if (true == CAS(0, ST_WAITING, 0, ST_EMPTY)) {  //slot = 0;
						*time_out = true;
						return 0;
					}
					else {	// 실패하면 교환이 된것
			...
			case ST_WAITING:
				ret = slot & 0x3FFFFFFF;
				if (true == CAS(ret, ST_WAITING, value, ST_BUSY))
					return ret;
